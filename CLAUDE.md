...
- **Deep Gradients:** Use rich, deep gradients for main containers.
  - _Example:_ `bg-gradient-to-b from-[#020617] via-[#0f172a] to-[#1e1b4b]`.
...
- **Script Handling:** NEVER render native `<script>` tags inside component trees. Use the `next/script` component.
  - _Placement:_ Keep scripts outside the fragment root or at the end of the `<body>`. Avoid placing them inside `<head>` in `layout.tsx` unless using the `beforeInteractive` strategy.
