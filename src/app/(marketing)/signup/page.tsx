...
import { ArrowRight, BusFront, Compass, Users } from "lucide-react";
...
const features = [
  {
    ...
    description: "Book rides, explore itineraries and submit reviews after the trip.",
    accent: { iconBg: "bg-cyan-400", bar: "from-cyan-400 to-sky-500", glow: "bg-cyan-300/20" },
  },
  {
    icon: BusFront,
    title: "Driver operations",
    description: "Driver accounts are provisioned internally so access stays controlled.",
    accent: { iconBg: "bg-amber-400", bar: "from-amber-400 to-orange-500", glow: "bg-amber-300/20" },
  },
  {
    icon: Users,
    title: "Admin oversight",
    description: "A seeded super admin account is available for platform setup and testing.",
        ...
        <div className="flex-1 space-y-7 overflow-hidden rounded-[2rem] border border-slate-200/80 bg-white/80 px-8 py-10 shadow-[0_40px_120px_rgba(148,163,184,0.18)] backdrop-blur-sm
          ...
          <div className="space-y-3">
            <h1 className="font-heading max-w-xl text-4xl font-extrabold leading-tight tracking-tight sm:text-5xl">
              ...
              coordination and guest service from one app.
            </h1>
            ...
            {features.map((f, i) => {
              ...
              return (
                ...
                >
                  ...
                  <h2 className="mt-4 text-sm font-bold text-slate-950">{f.title}</h2>
                  <p className="mt-1.5 text-xs leading-6 text-slate-500">{f.description}</p>
                </div>
              );
            })}
          </div>
